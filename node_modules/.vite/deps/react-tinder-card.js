...
// node_modules/p-sleep/index.js
var require_p_sleep = __commonJS({
  "node_modules/p-sleep/index.js"(exports, module) {
...
var require_react_tinder_card = __commonJS({
  "node_modules/react-tinder-card/index.js"(exports, module) {
    ...
    var normalize = (vector, multiplier = 1) => {
      const length = Math.sqrt(Math.pow(vector.x, 2) + Math.pow(vector.y, 2));
      return { x: vector.x * multiplier / length, y: vector.y * multiplier / length };
    };
    var animateOut = async (element, speed, easeIn = false) => {
      ...
      await sleep(time * 1e3);
    };
    var animateBack = async (element) => {
    ...
    var dragableTouchmove = (coordinates, element, offset, lastLocation) => {
      ...
      const rotCalc = calcSpeed(lastLocation, newLocation).x / 1e3;
      const rotation = rotationString(rotCalc * settings.maxTilt);
      element.style.transform = translation + rotation;
      return newLocation;
    };
    var touchCoordinatesFromEvent = (e) => {
      const touchLocation = e.targetTouches[0];
      return { x: touchLocation.clientX, y: touchLocation.clientY };
    };
    var mouseCoordinatesFromEvent = (e) => {
      return { x: e.clientX, y: e.clientY };
    };
    var TinderCard = React.forwardRef(({ flickOnSwipe = true, children, onSwipe, onCardLeftScreen, className, preventSwipe = [], swipeRequirementType = "velocity", swipeThreshold = settings.swipeThreshold, onSwipeRequirementFulfilled, onSwipeRequirementUnfulfilled }, ref) => {
      ...
      const handleSwipeReleased = React.useCallback(async (element2, speed) => {
        if (swipeAlreadyReleased.current) {
          return;
        }
        swipeAlreadyReleased.current = true;
        const currentPostion = getTranslate(element2);
        ...
        if (dir !== "none") {
          ...
          if (flickOnSwipe) {
            if (!preventSwipe.includes(dir)) {
              ...
              await animateOut(element2, outVelocity);
...
// dep:react-tinder-card
var react_tinder_card_default = require_react_tinder_card();
export {
  react_tinder_card_default as default
};
